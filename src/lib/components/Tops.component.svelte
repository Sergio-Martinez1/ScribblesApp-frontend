...
<div
	class="px-4 py-2.5 flex flex-col items-start gap-y-2 bg-purpleGray rounded-2xl m-auto max-h-fit min-w-full"
>
	{#if loading}
		<div class="animate-pulse bg-purpleLight rounded-2xl w-32 h-6" />
		<div class="flex flex-col items-start gap-y-2.5 w-full">
			{#each Array(3) as _}
				<div class="animate-pulse bg-purpleLight rounded-2xl w-full h-10 py-2 pl-4" />
			{/each}
		</div>
	{:else}
