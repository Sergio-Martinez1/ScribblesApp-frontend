...
<div class="relative w-20 h-8 flex justify-center items-center">
	<button
		value="default"
		type="submit"
		on:click
		...
		class="{error ? 'bg-squeezeRed' : 'bg-krispyPurple'} {disabled || isLoading
			? ''
			: 'active:bg-krispyPurple'}  hover:bg-lessPurple h-8 p-2.5 rounded-full flex items-center justify-center disabled:cursor-not-allowed disabled:opacity-50 {error
			? 'hover:bg-squeezeRed active:bg-squeezeRed w-16 h-8'
			: 'w-20'} {animations ? 'transition-all duration-400' : ''}"
	>
	...
	{#if errorMessage}
		<span
			...
			class="text-squeezeRed absolute text-sm w-28 flex justify-center font-bold top-[95%] z-20"
			>Please reload</span
		>
	{/if}
</div>
