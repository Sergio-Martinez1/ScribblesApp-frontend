...
>
	...
	<button class="bg-krispyPurple dark:text-white rounded-xl w-32 h-10" type="submit" on:click|preventDefault={handleSubmit}>
		<span class="px-auto font-bold text-white">Delete</span>
	</button>
</form>

	<dialog class="bg-lavandaGray dark:bg-purpleGray rounded-2xl shadow-[0px_0px_10px_1px_black]" id="confirmationDialog">
		<form>
          ...
          on:click={confirmSubmission}
					formmethod="dialog"
					class="bg-lavandaLight dark:bg-purpleLight hover:bg-hoverPurple dark:text-white p-2.5 rounded-2xl w-full"
				>
					Confirm
				</button>
				<button
